...
function CurrencyRow({
  ...
  return (
    <MenuItem style={style} className={`token-item-${key}`} onClick={() => (isSelected ? null : onSelect())} disabled={isSelected} selected={otherSelected}>
      ...
      <Column>
        <Text title={currency.name} style={{ margin: 0, fontWeight: 500, textAlign: 'left' }}>
          {currency.symbol}
        </Text>
        <FadedSpan>
          {!isOnSelectedList && customAdded && !(currency instanceof WrappedTokenInfo) ? (
            <Text style={{ margin: 0, fontWeight: 500, textAlign: 'left' }}>
              Added by user
              <LinkStyledButton
                onClick={(event) => {
      ...
      <RowFixed style={{ justifySelf: 'flex-end' }}>{balance ? <Balance balance={balance} /> : account ? <Loader /> : null}</RowFixed>
    </MenuItem>
  )
}
