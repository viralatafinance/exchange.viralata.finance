    ...
    <>
      ...
      <AppBody>
        <Wrapper id="swap-page">
          ...
          />
          <PageHeader title={TranslateString(8, 'Swap')} />
          <CardBody style={{ padding: 0, marginTop: 20, marginBottom: 10 }}>
            <AutoColumn gap="md">
              <CurrencyInputPanel
                  ...
                  <AutoColumn gap="4px">
                    {Boolean(trade) && (
                      <RowBetween align="center">
                        <Text style={{ margin: 0, fontSize: '14px' }}>{TranslateString(1182, 'Price')}</Text>
                        <TradePrice price={trade?.executionPrice} showInverted={showInverted} setShowInverted={setShowInverted} />
                      </RowBetween>
                    )}
                    {allowedSlippage !== INITIAL_ALLOWED_SLIPPAGE && (
                      <RowBetween align="center">
                        <Text style={{ margin: 0, fontSize: '14px' }}>{TranslateString(88, 'Slippage Tolerance')}</Text>
                        <Text style={{ margin: 0, fontSize: '14px' }}>{allowedSlippage / 100}%</Text>
                      </RowBetween>
                    )}
                  </AutoColumn>
                </Card>
            ...
            <BottomGrouping>
              {!account ? (
                ...
                <ButtonCTA disabled={Boolean(wrapInputError)} onClick={onWrap}>
                  {wrapInputError ?? (wrapType === WrapType.WRAP ? 'Wrap' : wrapType === WrapType.UNWRAP ? 'Unwrap' : null)}
                </ButtonCTA>
              ) : noRoute && userHasSpecifiedInputOutput ? (
                <GreyCard style={{ textAlign: 'center' }}>
                  <Text style={{ margin: 0, fontSize: '14px' }}>{TranslateString(1194, 'Insufficient liquidity for this trade.')}</Text>
                </GreyCard>
              ) : showApproveFlow ? (
                <RowBetween>
                  <ButtonCTA onClick={approveCallback} disabled={approval !== ApprovalState.NOT_APPROVED || approvalSubmitted}>
                  ...
                  <ButtonCTA
                    ...
                    iconRight={isValid && priceImpactSeverity > 2 ? <AlertTriangle style={{ color: '#fff' }} size={24} /> : null}
                  >
                ...
                <ButtonCTA
                  ...
                  iconRight={isValid && priceImpactSeverity > 2 ? <AlertTriangle style={{ color: '#fff' }} size={24} /> : null}
                >
                  {swapInputError || (priceImpactSeverity > 3 && !isExpertMode ? `Price Impact Too High` : `Swap${priceImpactSeverity > 2 ? ' Anyway' : ''}`)}
                </ButtonCTA>
              )}
